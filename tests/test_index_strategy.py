...
@pytest.mark.parametrize("value", ["first-index", "unsafe-first-match", "unsafe-best-match"])
...
def test_model_config_beats_the_machine_wide_env(monkeypatch):
    ...
    assert _index_strategy({"pip_index_strategy": "unsafe-best-match"}) == ("unsafe-best-match")
...
def test_single_url_string_and_list_both_work():
    ...
    assert _extra_index_urls({"pip_extra_index_url": ["https://a/x", "https://b/y"]}) == [
        "https://a/x",
        "https://b/y",
    ]
