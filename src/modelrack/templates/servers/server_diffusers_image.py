"""Image-generation inference server template (diffusers).

Runs inside the model's isolated .venv. Customize ``load_model()`` and
``run_inference()`` (e.g. Qwen-Image, Z-Image-Turbo). Do not modify the scaffolding.
...
def load_model(model_dir: Path, config: dict[str, Any]) -> Any:
    """Load and return the diffusers pipeline.
    ...
    """
    raise NotImplementedError("Customize load_model() for your image model")


def run_inference(model: Any, payload: dict[str, Any]) -> dict[str, Any]:
    """Generate an image and return it (base64 PNG or a saved path).
    ...
    Example:
        ...
        return {"image_base64": _png_b64(image)}
    """
    raise NotImplementedError("Customize run_inference() for your image model")
