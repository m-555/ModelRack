"""Qwen3-Coder-Next (FP8) code-LLM inference server — concrete implementation (vLLM).

Runs inside this model's isolated .venv (created by `modelrack setup qwen3-coder`).
Depends on vllm/torch, which are NOT installed in the modelrack hub env. The FP8
checkpoint needs an FP8-capable GPU (Hopper/Ada+) and a recent vLLM.
...
Request payload (merged config defaults + request params) -- provide messages OR prompt:
    ...
    temperature, top_p, top_k, min_p, presence_penalty, repetition_penalty, max_tokens, stop, seed
...
# vLLM accepts these as `dtype`; "fp8" is a quantization (serving.quantization), not a dtype.
...
def load_model(model_dir: Path, config: dict[str, Any]) -> Any:
    """Construct the vLLM engine (FP8 quantization) from this model's serving config."""
    serving = config.get("serving", {})
    hardware = config.get("hardware", {})
    dtype = hardware.get("dtype", "auto")
    if dtype not in _VALID_DTYPES:  # e.g. "fp8" is handled via `quantization`, not `dtype`
        dtype = "auto"
    ...
    return LLM(
        ...
        quantization=serving.get("quantization", "fp8"),
...
def run_inference(model: Any, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if "messages" in payload:
        outputs = model.chat(payload["messages"], sp)
...
def _main() -> None:
    ...
    if not args.lazy:
        print(f"Loading Qwen3-Coder (vLLM) from {model_dir} ...", flush=True)
        model = load_model(model_dir, config)
        _loaded_at = time.time()
