"""Qwen-Image (2512) text-to-image inference server — concrete implementation.

Runs inside this model's isolated .venv (created by `modelrack setup qwen-image`).
...
Request payload (merged config defaults + request params):
    prompt              (str, required)
    negative_prompt     (str)
    aspect_ratio        (str)   one of the presets below (sets width/height)
    width, height       (int)   override the aspect-ratio preset if given
    num_inference_steps (int)   default 50
    true_cfg_scale      (float) default 4.0
    seed                (int)   -1 for random

Response data: {"image_base64", "width", "height"}  (base64-encoded PNG)
"""
...
# Preset aspect ratios (width, height) supported by Qwen-Image-2512.
ASPECT_RATIOS = {
    "1:1": (1328, 1328),
    "16:9": (1664, 928),
    "9:16": (928, 1664),
...
def load_model(model_dir: Path, config: dict[str, Any]) -> Any:
    """Load the Qwen-Image pipeline from the local weights directory."""
    hardware = config.get("hardware", {})
    serving = config.get("serving", {})
    dtype = _DTYPES.get(hardware.get("dtype", "bfloat16"), torch.bfloat16)
    device = hardware.get("device", "cuda")

    pipe = DiffusionPipeline.from_pretrained(str(model_dir / "weights"), torch_dtype=dtype)
...
def run_inference(model: Any, payload: dict[str, Any]) -> dict[str, Any]:
    """Generate an image and return it as a base64 PNG."""
    if "prompt" not in payload:
        raise ValueError("payload.prompt is required")

    # Explicit width/height win; otherwise derive from the aspect-ratio preset.
    preset = ASPECT_RATIOS.get(payload.get("aspect_ratio", "1:1"), (1328, 1328))
    width = int(payload.get("width", preset[0]))
    height = int(payload.get("height", preset[1]))
    ...
    return {"image_base64": _png_b64(image), "width": width, "height": height}
...
def _main() -> None:
    ...
    if not args.lazy:
        print(f"Loading Qwen-Image from {model_dir} ...", flush=True)
        model = load_model(model_dir, config)
        _loaded_at = time.time()
