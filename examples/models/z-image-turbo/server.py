"""Z-Image Turbo (text-to-image) inference server — concrete implementation.

Runs inside this model's isolated .venv (created by `modelrack setup z-image-turbo`).
...
Request payload (merged config defaults + request params):
    prompt              (str, required)
    num_inference_steps (int)   default 9 (distilled — ~8 DiT forwards)
    guidance_scale      (float) default 0.0 (MUST be 0 for Turbo variants)
...
def load_model(model_dir: Path, config: dict[str, Any]) -> Any:
    """Load the Z-Image Turbo pipeline from the local weights directory."""
    ...
    pipe = ZImagePipeline.from_pretrained(
        str(model_dir / "weights"), torch_dtype=dtype, low_cpu_mem_usage=False
    )
...
def run_inference(model: Any, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    image = model(
        ...
        height=height,
        width=width,
        num_inference_steps=int(payload.get("num_inference_steps", 9)),
...
def _main() -> None:
    ...
    if not args.lazy:
        print(f"Loading Z-Image Turbo from {model_dir} ...", flush=True)
        model = load_model(model_dir, config)
        _loaded_at = time.time()
